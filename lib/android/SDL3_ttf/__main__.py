...
"""
Create a SDL3_ttf SDK prefix from an Android archive
This file is meant to be placed in a the root of an android .aar archive

Example usage:
```sh
python SDL3_ttf-3.2.2.aar -o /usr/opt/android-sdks
cmake -S my-project \
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert a SDL3_ttf Android .aar archive into a SDK",
        allow_abbrev=False,
    )
    parser.add_argument("--version", action="version", version="SDL3_ttf 3.2.2")
    parser.add_argument("-o", dest="output", type=pathlib.Path, required=True, help="Folder where to store the SDK")
    args = parser.parse_args()

    print(f"Creating a SDL3_ttf SDK at {args.output}...")
