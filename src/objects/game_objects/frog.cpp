...
Frog::Frog(const int size, const int screen_width, const int screen_height)
        : size(size), m_screen_width(screen_width), m_screen_height(screen_height),
          x(screen_width / 2), y(screen_height - size),
          m_draw_objects(
                  std::make_unique<std::vector<std::unique_ptr<DrawObjectBase>>>()) {
    create_draw_objects();
}
...
void Frog::update() {
    ...
    if (y < 0)
        y = 0;
    if (y > m_screen_height - size)
        y = m_screen_height - size;
    if (x > m_screen_width - size)
        x = m_screen_width - size;
...
std::unordered_map<SDL_Keycode, std::function<void()>>
Frog::get_key_down_map() {
...
void Frog::set_y(int y_in) { y = y_in; }

void Frog::start_move_right() { move_right = true; }

void Frog::start_move_left() { move_left = true; }

void Frog::start_move_down() { move_down = true; }

void Frog::start_move_up() { move_up = true; }

void Frog::stop_move_left() { move_left = false; }

void Frog::stop_move_right() { move_right = false; }

void Frog::stop_move_down() { move_down = false; }

void Frog::create_draw_objects() {
    m_draw_objects->clear();
    const Color green({0, 224, 0, 255});
    m_draw_objects->push_back(
            std::make_unique<DrawObjectRect>(x, y, size, size, green, true));
}

void Frog::stop_move_up() { move_up = false; }

Rectangle Frog::get_rect() { return Rectangle(
