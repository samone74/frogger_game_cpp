...
public:
    Car(int x, int y, int speed, Color color, int width, int height, int screen_width,
        int screen_height);
    ...
    get_key_down_map() override { return {}; }
...
private:
    ...
    int m_speed;
    const Color m_color;
    std::unique_ptr<std::vector<std::unique_ptr<DrawObjectBase>>> m_draw_objects;
