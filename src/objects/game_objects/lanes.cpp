...
ObjectBase::Type Lanes::get_type() const { return Type::Lanes; }
...
void Lanes::create_draw_objects() {
    ...
    m_draw_objects->push_back(std::make_unique<DrawObjectRect>(
            0, m_screen_height / 2 - m_lane_size / 2 * m_number_of_lanes,
            m_screen_width, m_lane_size * m_number_of_lanes, gray, true));
    for (int i = 0; i < m_number_of_lanes + 1; i++) {
        m_draw_objects->push_back(std::make_unique<DrawObjectRect>(
                0,
                m_screen_height / 2 - m_lane_size / 2 * m_number_of_lanes +
                m_lane_size * (i) - line_size / 2,
...
Rectangle Lanes::get_rect() {
    return Rectangle({0,
                      ...
                      static_cast<float>( m_lane_size) / 2 * static_cast<float>( m_number_of_lanes),
