...
Car::Car(const int x, const int y, const int speed, const Color color,
        ...
        : m_x(x), m_y(y), m_speed(speed), m_color(color), m_width(width),
          m_height(height), m_screen_width(screen_width),
...
void Car::create_car_draw_objects() {
    ...
    m_draw_objects->push_back(std::make_unique<DrawObjectRect>(
            m_x, m_y, m_width, m_height, m_color, true));
    // front rectangle
    m_draw_objects->push_back(std::make_unique<DrawObjectRect>(
            m_x, m_y, m_width / 4, m_height, black, false));
    // middle rectangle
    m_draw_objects->push_back(std::make_unique<DrawObjectRect>(
            m_x + m_width / 4, m_y, m_width / 2, m_height, black, false));
    // back rectangle
    m_draw_objects->push_back(std::make_unique<DrawObjectRect>(
            m_x + 3 * m_width / 4, m_y, m_width / 4, m_height, black, false));
}
...
Rectangle Car::get_rect() { return Rectangle(
            {static_cast<float>(m_x), static_cast<float>(m_y), static_cast<float>(m_width),
